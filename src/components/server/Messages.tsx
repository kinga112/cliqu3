...
export function Messages(){
  ...
  const appendOldMessages = useServerStore((state) => state.appendOldMessages)
  const textChannels = useServerStore(server => server.textChannels)
  const setTextChannels = useServerStore(server => server.setTextChannels)
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (scrollRef.current) {
        ...
        // console.log("SCROLL TOP: ", Math.round(scrollRef.current.scrollTop))
        // console.log("SCROLL MAX: ", Math.round(max))
  ...
  return(
    <>
      <div className="flex flex-grow"/> {/* spacer to push messages to bottom */}
      <div ref={scrollRef} className="flex flex-col overflow-y-auto">
        <div key={currentTextChannel.chatId} className="w-full flex flex-col flex-grow min-h-0 mb-2 pb-6 pt-4"> {/* mb-2 to prevent scroll on not overflow ... idk tbh */}
          {messageList}
        </div>
      </div>
