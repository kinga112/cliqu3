...
export function UserInfoLarge(props: {address: string, userProfile: UserProfile}){
  ...
  async function unblockUser(){
    push.user!.chat.unblock([address]).then((user: IUser) => {
      ...
      {/* { props.edit ? */}
        ...
        <div className="relative w-96 h-36 rounded-xl overflow-hidden pointer-events-none z-50">
          ...
          { myProfile ? 
            <div/> :
              blocked ?
                <button className="absolute flex justify-center bottom-5 right-5 bg-deep-purple-100 rounded-lg z-50 pointer-events-auto hover:bg-deep-purple-200" onClick={unblockUser}>
                  <img className="w-8 h-8" src={unblock}/>
                </button> :
                <button className="absolute bottom-5 right-5 p-1 bg-deep-purple-100 rounded-lg z-50 pointer-events-auto hover:bg-deep-purple-200" onClick={blockUser}>
