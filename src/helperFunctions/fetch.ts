...
// export async function    fetchHistory(chatId: string, reference: string|null = null): Promise<[Msg[], string]>{
export async function newFetchHistory(chatId: string, reference: string = ''): Promise<[boolean, string]>{
  ...
  for(i; i<history.length;i++){
    ...
    if(channel.lastReadMessageCid == pushMsg.cid){
      ...
      cache.updateLastReadMessageCid(chatId, pushMsg.cid)
    }
    ...
    if(pushMsg.messageType == 'Reaction'){
      ...
      const message: Message = {
        ...
        from: from,
        // message: { type: pushMsg.message.type, content: {type: pushMsg.message.content.type, content: pushMsg.message.content.content}, reference: pushMsg.message.reference },
        message: { 
      ...
      // console.log("SUCCESS? type: reply " + success)
      // messages.push(message)
      if(success){
      ...
      // messages.push(message)
      console.log("SUCCESS? type: else: " + success)
      // if(!success){
      //   return [success, '']
      // }
    }
